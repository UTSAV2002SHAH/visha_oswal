...
export async function POST(req: NextRequest) {
    try {
        ...
        if (user) {
            ...
            if (userId === user) {
                return NextResponse.json({ error: "Cannot link yourself as a spouse" }, { status: 400 });
            }
        ...
        // 2. Update the user's MemberProfile (Overwrites any existing spouse)
        const spouseData = {
