...
export async function POST(req: NextRequest) {
    try {
        ...
        if (user) {
            ...
            if (userId === user) {
                return NextResponse.json({ error: "Cannot link yourself as a sibling" }, { status: 400 });
            }
        ...
        // 2. Add sibling to the user's MemberProfile (Pushes to array instead of overwriting)
        const siblingData = {
        ...
        const updatedProfile = await MemberProfile.findOneAndUpdate(
            { userId },
            { $push: { "family.siblings": siblingData } },
